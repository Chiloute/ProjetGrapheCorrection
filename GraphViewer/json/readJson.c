...
typed(json_element) interpretJSon(const char *fileName) {
  ...
  if (json == NULL) {
    enum json_element_type_e null_element_kind = JSON_ELEMENT_TYPE_NULL;
    typed(json_element) null_element = {.type = null_element_kind};
    return null_element;
  }
...
bool hasElementWithName(typed(json_element) element, const char *nodeName) {
  result(json_element) result =
      json_object_find(element.value.as_object, nodeName);
  return !(result_is_err(json_element)(&result));
};

typed(json_element)
    getJsonElementFromName(typed(json_element) element, const char *nodeName) {
  result(json_element) result =
      json_object_find(element.value.as_object, nodeName);
