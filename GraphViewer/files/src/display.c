...
void displayGraphWindow(Graph *graph) {
  ...
  char editedText[MAX_INPUT_CHARS + 1] =
  ...
  while (!WindowShouldClose()) // Detect window close button or ESC key
  {
    ...
    prepareGraphToDraw(graph, startPoint, &pressed, &displayEdgeWeightEditBox,
                       &textBox, &letterCount, editedText, &editedEges,
                       &framesCounter, &creatingNode, &movingNode, &mouse,
                       &createEdge);

    // Draw the graph
    drawGraph(displayEdgeWeightEditBox, &textBox, editedText, letterCount,
              framesCounter, &currentPoint, &startPoint, graph, moveStartPoint,
              movingNode, pressed, &mouse, &createEdge);
...
void prepareGraphToDraw(Graph *graph, Vector2 startPoint, int *pressed,
                        int *show, Rectangle *textBox, int *letterCount,
                        char *editedText, NodesList **editedEges,
                        int *framesCounter, int *creatingNode,
  ...
  while (currentNodeList->next != NULL) {
    ...
    if (deleteNodeManagement(mouse, node, prevNodeList, currentNodeList,
                             graph) ||
        deleteEdgeManagement(graph, node, mouse, editedEges))
      break;

    prevNodeList = currentNodeList;

    createNodeManagement(creatingNode, graph, mouse);
    manageNodeRenameEditBox(graph, node, show, textBox, mouse, letterCount,
                            editedText, framesCounter, &overNode, nodeToEdit);

    if (*nodeToEdit == NULL || *createEdge) {
      manageEdgeWeightEditBox(graph, node, show, textBox, mouse, letterCount,
                              editedText, framesCounter, &overEdge, editedEges);

      // If the mouse is over a node, the user can move it by left clicking on
      // it
      if (CheckCollisionPointCircle(*mouse, (Vector2){node->x, node->y},
                                    50.0f) &&
      ...
      if (*nodeToEdit != NULL) {
        ...
        // Once the user has moved the node, he can release it by releasing the
        // left mouse button
        if (IsMouseButtonReleased(MOUSE_BUTTON_LEFT))
          (*nodeToEdit) = NULL;
      }
      // If no node is being moved check the next one, but if a node is being
      // moved, stop checking
      if ((*nodeToEdit) == NULL)
...
bool deleteNodeManagement(Vector2 *mouse, Node *node, NodesList *prevNodeList,
                          NodesList *currentNodeList, Graph *graph) {
  if (IsMouseButtonDown(MOUSE_BUTTON_LEFT) && IsKeyDown(KEY_LEFT_ALT)) {
    bool overNode =
        CheckCollisionPointCircle(*mouse, (Vector2){node->x, node->y}, 50.0f);
    if (overNode) {
      if (prevNodeList)
...
bool deleteEdgeManagement(Graph *graph, Node *node, Vector2 *mouse,
                          NodesList **editedEges) {
...
void createNodeManagement(int *creatingNode, Graph *graph, Vector2 *mouse) {
  if (IsKeyDown(KEY_LEFT_CONTROL) && IsMouseButtonDown(MOUSE_BUTTON_LEFT) &&
    ...
    sprintf(buffer, "%d", graph->nodesCount);
...
void manageNodeRenameEditBox(Graph *graph, Node *node, int *show,
                             Rectangle *textBox, Vector2 *mouse,
                             int *letterCount, char *editedText,
                             int *framesCounter, bool *overNode,
                             Node **nodeToEdit) {
  ...
  if ((currentGesture == GESTURE_DOUBLETAP && *overNode) ||
      (*show && *nodeToEdit != NULL)) {
    if (*nodeToEdit == NULL)
      *nodeToEdit = node;

    bool edited = false;
    getUserInput(show, &edited, textBox, mouse, letterCount, editedText,
                 framesCounter, true);
...
// If the graph is oriented, the function will check if the mouse is over the
// triangle indicated the orientation If the graph is not oriented, the function
// will check if the mouse is over anypart of the edge
bool checkOverEdge(bool oriented, Node *node, Vector2 *mouse,
                   NodesList **editedEges) {
  bool overEdge = false;
  NodesList *adjencyList = node->adjacent;
  ...
  while (adjencyList != NULL && adjencyList->node != NULL) {
    if (!oriented) {
      if (CheckCollisionPointLine(
              *mouse, (Vector2){node->x, node->y},
              (Vector2){adjencyList->node->x, adjencyList->node->y}, 5)) {
        overEdge = true;
        break;
      }
    } else {
      ...
      Vector2 normalNormalized =
          Vector2Normalize((Vector2){normal.x, normal.y});
      ...
      Vector2 middle = {
          (node->x + adjencyList->node->x) / 2 + normalNormalized.x,
          (node->y + adjencyList->node->y) / 2 + normalNormalized.y};

      Vector2 colinear =
          Vector2Normalize((Vector2){(adjencyList->node->x - node->x) / 2,
      ...
      Vector2 triangle[3] = {middle, Vector2Add(backVertice, normalNormalized),
                             Vector2Subtract(backVertice, normalNormalized)};

      if (CheckCollisionPointTriangle(*mouse, triangle[0], triangle[1],
                                      triangle[2])) {
        overEdge = true;
...
void getUserInput(int *show, bool *edited, Rectangle *textBox, Vector2 *mouse,
                  int *letterCount, char *editedText, int *framesCounter,
                  bool acceptChar) {
  if (!(*show)) {
    (*textBox).x = mouse->x;
    (*textBox).y = mouse->y;
...
void manageEdgeWeightEditBox(Graph *graph, Node *node, int *show,
                             Rectangle *textBox, Vector2 *mouse,
                             int *letterCount, char *editedText,
                             int *framesCounter, bool *overEdge,
                             NodesList **editedEges) {
  int currentGesture = GetGestureDetected();
...
void edgeCreationManagement(Node **nodeToMove, Vector2 *mouse, int *createEdge,
                            Graph *graph) {
  if (!IsKeyDown(KEY_LEFT_SHIFT)) {
    ...
    if (*createEdge != 0) {
      ...
      while (tmpNodeList->node != NULL) {
        if (CheckCollisionPointCircle(
                *mouse, (Vector2){tmpNodeList->node->x, tmpNodeList->node->y},
                50.0f)) {
...
void drawGraph(int showEdgeWeightEditBox, Rectangle *textBox, char *edgeWeight,
               int letterCount, int framesCounter, Vector2 *currentPoint,
               Vector2 *startPoint, Graph *graph, int moveStartPoint,
               Node *movingNode, int pressed, Vector2 *mouse, int *createEdge) {
  BeginDrawing();
  ...
  while (graphNodes->next != NULL) {
    ...
    while (adjencyList != NULL && adjencyList->node != NULL) {
      Vector2 middle =
          Vector2Add((Vector2){node->x, node->y},
                     (Vector2){adjencyList->node->x, adjencyList->node->y});
      middle = Vector2Scale(middle, 0.5);
      if (!graph->oriented)
        DrawLine((int)node->x, (int)node->y, (int)adjencyList->node->x,
                 (int)adjencyList->node->y, GREEN);
      else
        drawOrientedCurve(node, adjencyList, &middle);

      DrawText(TextFormat("%d", adjencyList->weight), (int)middle.x,
               (int)middle.y, 30, RED);
  ...
  while (graphNodes->next != NULL) {
    ...
    DrawCircleV(*currentPoint,
                CheckCollisionPointCircle(*mouse, *currentPoint, 30.0f) &&
                        !IsKeyDown(KEY_LEFT_SHIFT)
                    ? 50.f
                    : 30.f,
                (moveStartPoint != -1 && movingNode == node) ? RED : BLUE);
    DrawText(TextFormat("%s", node->id), (int)currentPoint->x - 5,
             (int)currentPoint->y - 15, 30, WHITE);
    graphNodes = graphNodes->next;
  }
  ...
  if (showEdgeWeightEditBox)
    drawEdgeWeightEditBox(showEdgeWeightEditBox, textBox, edgeWeight,
                          letterCount, framesCounter);

  EndDrawing();
}

void drawEdgeWeightEditBox(int show, Rectangle *textBox, char *edgeWeight,
                           int letterCount, int framesCounter) {
  DrawRectangleRec(*textBox, LIGHTGRAY);
  DrawRectangleLines((int)textBox->x, (int)textBox->y, (int)textBox->width,
                     (int)textBox->height, RED);
  DrawText(edgeWeight, (int)textBox->x + letterCount * 5, (int)textBox->y + 8,
           40, MAROON);

  if (letterCount < MAX_INPUT_CHARS) {
    // Draw blinking underscore char
    if (framesCounter > 50)
      DrawText("_", (int)textBox->x + 8 + MeasureText(edgeWeight, 40),
               (int)textBox->y + 12, 40, MAROON);
  } else
    DrawText("Press BACKSPACE to delete chars...", (int)textBox->x,
             (int)textBox->y - 20, 20, GRAY);
}
...
void drawOrientedCurve(Node *node, NodesList *adjencyList, Vector2 *middle) {
  ...
  *middle =
      (Vector2){(node->x + adjencyList->node->x) / 2 + normalNormalized.x,
                (node->y + adjencyList->node->y) / 2 + normalNormalized.y};
  ...
  colinear.x *= multiplier;
  colinear.y *= multiplier;
  // DrawLine(node->x, node->y, node->x + control1.x, node->y + control1.y,
  // PURPLE); DrawLine(adjencyList->node->x, adjencyList->node->y,
  //	adjencyList->node->x + control1.x, adjencyList->node->y + control1.y,
  // PURPLE);
  DrawSplineSegmentBezierCubic(
      (Vector2){node->x, node->y},
      (Vector2){node->x + normalNormalized.x, node->y + normalNormalized.y},
      (Vector2){middle->x, middle->y}, *middle, 2, RED);
  DrawSplineSegmentBezierCubic(
      *middle, (Vector2){middle->x, middle->y},
      (Vector2){adjencyList->node->x + normalNormalized.x,
                adjencyList->node->y + normalNormalized.y},
      (Vector2){adjencyList->node->x, adjencyList->node->y}, 2, RED);
  Vector2 backVertice = Vector2Subtract(*middle, colinear);
  ...
  DrawTriangle(*middle, Vector2Add(backVertice, normalNormalized),
